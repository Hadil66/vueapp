                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <v-card-subtitle
                            class="pt-3 pb-1 text-center meeting-room-name"
                          >
                            {{ ruimte.name }}
                          </v-card-subtitle>
                  ...
                  <v-row>
                    <v-col cols="12" md="4">
                      ...
                      >
                        ...
                        <v-card-text
                          class="text-body-2 text-center pb-1"
                          v-html="selectedRoomObject.description"
                        ></v-card-text>
                        <v-card-text class="text-body-2 text-center pt-0">
                          Capaciteit:
                          <strong>{{ selectedRoomObject.capacity }} personen</strong>
                        </v-card-text>
                        <v-card-text class="text-body-2 text-center pt-0">
                          Features:
                          <strong>{{ selectedRoomObject.features.join(", ") }}</strong>
                        </v-card-text>
                      </v-card>
                    </v-col>
                    <v-col cols="12" md="4">
                    ...
                    <v-col cols="12" sm="4">
                      ...
                      <v-list v-else density="compact" class="mt-2 time-list" lines="one">
                        ...
                        <p
                          v-if="
                            staticPossibleTimeSlots.length > 0 && allTimesBookedForDate
                          "
                        ...
                        <p
                          v-if="staticPossibleTimeSlots.length === 0 && !isLoadingTimes"
              ...
              <v-row justify="center">
                <v-col cols="12" md="8" lg="7">
                  <v-alert
                    ...
                    :text="
                      lastBookingDetails?.isApiBooking
...
// Functions communicating with the WordPress REST API
...
const processRoomBookings = () => {
  ...
  ruimtes.value.forEach((room) => {
    const relevantApiBookings = allBookingsFromApi.value.filter(
      (b) => b.roomId === room.id
    );
    ...
    relevantApiBookings.forEach((apiBooking) => {
      ...
      (apiBooking.times || []).forEach((time) =>
        roomBookingsByDate[apiBooking.date].add(time)
      ); 
    });

    room.bookings = Object.entries(roomBookingsByDate)
      .map(([date, timesSet]) => ({
        date: date,
...
const allTimesBookedForDate = computed(() => {
  if (
    ...
    !selectedDate.value ||
    staticPossibleTimeSlots.length === 0
  )
    return false;
...
const popularTimeSlots = computed(() => {
  const counts = {};
  let maxCount = 0;
  staticvPossibleTimeSlots.forEach((time) => (counts[time] = 0));
...
const submitBooking = async () => {
  ...
  if (!usernameFromEnv || !appPasswordFromConfig) {
    ...
    apiError.value =
      "Authenticatiegegevens ontbreken. Controleer de .env.local en config.js bestanden.";
...
// Store details for the confirmation screen
    ...
    }. Controleer de WordPress authenticatie en CORS instellingen.`;
  } finally {
    isSubmittingBooking.value = false;
  }
};
