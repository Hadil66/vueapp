<template>
  ...
  <div
    v-else-if="props.isLoadingTimes"
    class="pa-4 mt-2 text-grey-darken-1 text-center time-list-placeholder"
  >
  ...
  <v-list v-else density="compact" class="mt-2 time-list" lines="one">
    ...
    <v-list-item
      ...
      :disabled="props.isTimeSlotBookedFn(time)"
      @click="toggleTimeSelection(time)"
    >
      <template #prepend>
        <v-list-item-action start class="mr-2">
          <v-checkbox-btn
            :model-value="localTimeSelection.includes(time)"
            :disabled="props.isTimeSlotBookedFn(time)"
            readonly
          ></v-checkbox-btn>
        </v-list-item-action>
    ...
    <p
      v-if="props.staticPossibleTimeSlots.length > 0 && props.allTimesBookedForDate"
      class="text-caption text-orange text-center pa-4"
    >
      Alle tijden zijn volgeboekt op deze datum.
    </p>
    <p
      v-if="props.staticPossibleTimeSlots.length === 0 && !props.isLoadingTimes"
